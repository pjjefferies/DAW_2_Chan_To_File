...
def run_example():
    ...
    use_device_detection: bool = False
    dev_id_list = []
    board_num = 0
    rate: int = 2
    ...
    try:
        ...
        low_chan: int = 0
        high_chan: int = 1
        num_chans: int = high_chan - low_chan + 1
        ...
        print(f"points_per_channel {points_per_channel}")
        print(f"ai_info.packet_size {ai_info.packet_size}")
        print(f"ai_info.supported_ranges: {ai_info.supported_ranges}")
        ...
        ai_range = (4, -1.0, 1.0)  # BIP1VOLTS, -1 to +1 Volts
